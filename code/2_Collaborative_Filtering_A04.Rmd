---
title: "A04 Collaborative Filtering Algorithm"
...
User_RFM_A04 <- User_RFM

User_RFM_A04_test <- User_RFM_A04 %>% group_by(ID,PD_H_NM,PD_M_NM) %>% summarise(RFM_score = sum(RFM_score))
User_RFM_A04_test$RFM_score<-scale_min_max(User_RFM_A04_test$RFM_score)

test <- User_RFM_A04_test %>% group_by(ID,PD_H_NM)%>% summarise(RFM_score=round(sum(RFM_score),5))
test1 <- test %>% dcast(ID ~ PD_H_NM)
# test[is.na(test)]<-0

A04_PD_H_NM <- test1

# A04_PD_H_NM_2<-A04_PD_H_NM
# A04_PD_H_NM_2 <- ifelse(A04_PD_H_NM_2>0,1,0)
# apply(A04_PD_H_NM_2,2,sum,na.rm=T)
```

# 3. 유저 기반 협업필터링 알고리즘
```{r}
target <- colnames(A04_PD_H_NM)[-1] 
A04_item <- list()
A04_data.germany.user.scores <- list()
A04_data.germany.user.scores.holder <- list()
A04_all_buy_black <- list()
A04_target_user <- list()
A04_future_user <- list()

for (p in 1:length(target)){

A04_PD_H_NM_1 <-  User_RFM_A04 %>% subset(PD_H_NM == target[p]) %>% group_by(ID,PD_S_NM)%>% summarise(RFM_score=round(sum(RFM_score),5)) %>% dcast(ID ~ PD_S_NM)
A04_PD_H_NM_1 <- A04_PD_H_NM_1[,-1]
A04_PD_H_NM_1[is.na(A04_PD_H_NM_1)] <- 0
A04_PD_H_NM_1 <- ifelse(A04_PD_H_NM_1>0,1,0)
test_A04 <- apply(A04_PD_H_NM_1,2,sum) %>% as.data.frame 
test_A04$total <- nrow(A04_PD_H_NM_1)
test_A04$ratio <- round(test_A04$./test_A04$total,5)
test_A04 <- test_A04 %>% subset(ratio > 0.5) %>% rownames
A04_all_buy_black[[p]] <- test_A04
# A04_PD_H_NM_2<-A04_PD_H_NM
# A04_PD_H_NM_2 <- ifelse(A04_PD_H_NM_2>0,1,0)
# apply(A04_PD_H_NM_2,2,sum,na.rm=T)[-1]

target_user <- A04_PD_H_NM %>% select(ID,target[p])
target_user <- target_user[which(!is.na(target_user[,2])),]
A04_target_user[[p]] <- target_user[target_user[,2] > quantile(target_user[,2],0.75),]$ID
A04_future_user[[p]] <- target_user[target_user[,2] <= quantile(target_user[,2],0.75),]$ID

target_user <- SPI  %>% subset(BIZ_UNIT == "A04") %>% subset(ID %in% A04_target_user[[p]]) %>% subset(PD_H_NM==target[p]) %>% subset(!PD_S_NM %in% test_A04)
...
A04_item[[p]] <- data.germany.neighbours[,-1] %>% as.data.frame
...
################################################################ 
########### list : A04_data.germany.user.scores  ###############
################################################################
...
A04_data.germany.user.scores[[p]] <- holder

  
##########################################################################
##########  User Based Collaborative filtering - Item recommend  #########
########################################################################## 
############# list : A04_data.germany.user.scores.holder  ################
##########################################################################
...
A04_data.germany.user.scores.holder[[p]] <- data.germany.user.scores.holder
}
```
...
###################################################################### 
####################### list : A04_user_popular  #####################
######################################################################



A04_user_popular <-list()
A04_user_popular.holder <- list()

# 사던거 추천
for (p in 1:length(target)){
User_RFM_A04_S <- User_RFM_A04 %>% 
  subset(PD_H_NM==target[p]) %>% 
  subset(ID %in% A04_target_user[[p]]) %>% 
  dcast(ID ~ PD_S_NM)
User_RFM_A04_S[,-1] <- lapply(User_RFM_A04_S[,-1],function(x){ifelse(x<0.05,NA,x)})
A04_user_popular[[p]] <- User_RFM_A04_S
rownames(A04_user_popular[[p]]) <- paste0('u',A04_user_popular[[p]]$ID)
A04_user_popular[[p]] <- A04_user_popular[[p]][,-1]
}

#############################################################################
##########  Individual Popular Recommender System - Item recommend  #########
#############################################################################
####################### list : A04_user_popular.holder  #####################
#############################################################################

for (p in 1:length(target)){
  # Lets make our recommendations pretty
    data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A04_user_popular[[p]]),ncol=10,dimnames=list(rownames(nrow(A04_user_popular[[p]]))))

  for(i in 1:nrow(data.germany.user.scores.holder)) 
  {
    test_length <- A04_user_popular[[p]][,order(A04_user_popular[[p]][i,],decreasing=T)][i,] %>% length
    if(test_length>10) {
      test <- A04_user_popular[[p]][,order(A04_user_popular[[p]][i,],decreasing=T)][i,][1:10]
      }else{test <- head(n=10,A04_user_popular[[p]][,order(A04_user_popular[[p]][i,],decreasing=T)][i,])}
  ...
  A04_user_popular.holder[[p]] <- data.germany.user.scores.holder
  rownames(A04_user_popular.holder[[p]]) <- rownames(A04_user_popular[[p]])
...
# 5. A04 업종에서의 취향 순위
```{r}
A04_interest <- A04_PD_H_NM
A04_interest[,-1] <- apply(A04_interest[,-1],2,function(x)ifelse(x <= quantile(x,0.75,na.rm=T),NA,x))
A04_interest_s <- A04_interest
A04_interest_s[,-1] <- ifelse(A04_interest_s[,-1] > 0 ,1 ,0)
A04_interest$type_num <- apply(A04_interest_s[,-1],1,sum,na.rm=T)
A04_interest <- A04_interest %>% subset(type_num>0)

A04_interest_test <- A04_interest[,2:(ncol(A04_interest)-1)]
rownames(A04_interest_test) <- paste0('u',A04_interest$ID)

data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A04_interest_test),ncol=10,dimnames=list(rownames(nrow(A04_interest_test))))
 for(i in 1:nrow(data.germany.user.scores.holder)) {
    test_length <- A04_interest_test[,order(A04_interest_test[i,],decreasing=T)][i,] %>% length
    if(test_length>10) {
      test <- A04_interest_test[,order(A04_interest_test[i,],decreasing=T)][i,][1:10]
    }else{
        test <- head(n=10,A04_interest_test[,order(A04_interest_test[i,],decreasing=T)][i,])
        }
    
    data.germany.user.scores.holder[i,] <- c(names(test)[1:length(test[!is.na(test)])],rep(NA,10-length(names(test)[1:length(test[!is.na(test)])])))
}
  A04_interest.holder <- data.germany.user.scores.holder
  rownames(A04_interest.holder) <- rownames(A04_interest_test)
...
```{r} 
A04_interest.holder_S <- A04_interest.holder
for(i in 1:nrow(A04_interest.holder_S)){
    A04_interest.holder_S[i,]<-head(n=10,rep(A04_interest.holder_S[i,][!is.na(A04_interest.holder_S[i,])],10))
  }
  
 
data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A04_interest.holder_S),ncol=100,dimnames=list(rownames(nrow(A04_interest.holder_S))))
rownames(data.germany.user.scores.holder) <- rownames(A04_interest.holder_S)
A04_user_popular.holder_copy <- A04_user_popular.holder
A04_data.germany.user.scores.holder_copy <- A04_data.germany.user.scores.holder

for (i in 1:nrow(A04_interest.holder_S)){
  k=1
  for(j in 1:ncol(A04_interest.holder_S)){
    u <- names(A04_interest.holder_S[i,j])
    p <- which(A04_interest.holder_S[i,j]==target)
     if(sum(rownames(A04_user_popular.holder_copy[[p]]) %in% u)==1){
       if(!is.na(A04_user_popular.holder_copy[[p]][u,1])){
      data.germany.user.scores.holder[i,k] <- A04_user_popular.holder_copy[[p]][u,1]
      A04_user_popular.holder_copy[[p]][u,] <- c(A04_user_popular.holder_copy[[p]][u,][-1],NA)
      k=k+1
    }}
    if(sum(rownames(A04_data.germany.user.scores.holder_copy[[p]]) %in% u)==1){
      if(!is.na(A04_data.germany.user.scores.holder_copy[[p]][u,1])){
      data.germany.user.scores.holder[i,k] <- A04_data.germany.user.scores.holder_copy[[p]][u,1]
      A04_data.germany.user.scores.holder_copy[[p]][u,] <- c(A04_data.germany.user.scores.holder_copy[[p]][u,][-1],NA)
      k=k+1
    }}
  }
}
...
A04_total_recomm <- data.germany.user.scores.holder
```

# 7. EDA : A04 업종에서 취향(대분류)별 성별, 나이 요약 정보
```{r}
...
# 8. EDA : A04 업종에서 개인이 가진 취향의 개수 분포
```{r}
# A04_PD_H_NM_test <- A04_PD_H_NM
# A04_PD_H_NM_test[,-1] <- lapply(A04_PD_H_NM[,-1],function(x) ifelse(x>quantile(x,0.75,na.rm=T),1,0))
# A04_PD_H_NM_test[is.na(A04_PD_H_NM_test)]<-0
# A04_PD_H_NM_test$type_num <- apply(A04_PD_H_NM_test[,-1],1,sum)
# 
# A04_PD_H_NM_test %>% ggplot(aes(x=as.factor(type_num))) + geom_bar() +  xlab("A04")
# A04_PD_H_NM_test$type_num %>% table
```
