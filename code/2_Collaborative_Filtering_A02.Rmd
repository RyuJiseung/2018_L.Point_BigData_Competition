---
title: "A02 Collaborative Filtering Algorithm"
...
User_RFM_A02 <- User_RFM

User_RFM_A02_test <- User_RFM_A02 %>% group_by(ID,PD_H_NM,PD_M_NM) %>% summarise(RFM_score = sum(RFM_score))
User_RFM_A02_test$RFM_score<-scale_min_max(User_RFM_A02_test$RFM_score)

test <- User_RFM_A02_test %>% group_by(ID,PD_H_NM)%>% summarise(RFM_score=round(sum(RFM_score),5))
test1 <- test %>% dcast(ID ~ PD_H_NM)
# test[is.na(test)]<-0

A02_PD_H_NM <- test1

# A02_PD_H_NM_2<-A02_PD_H_NM
# A02_PD_H_NM_2 <- ifelse(A02_PD_H_NM_2>0,1,0)
# apply(A02_PD_H_NM_2,2,sum,na.rm=T)
```

# 3. 유저 기반 협업필터링 알고리즘
```{r}
target <- colnames(A02_PD_H_NM)[-1]
A02_item <- list()
A02_data.germany.user.scores <- list()
A02_data.germany.user.scores.holder <- list()
A02_all_buy_black <- list()
A02_target_user <- list()
A02_future_user <- list()

for (p in 1:length(target)){

A02_PD_H_NM_1 <-  User_RFM_A02 %>% subset(PD_H_NM == target[p]) %>% group_by(ID,PD_S_NM)%>% summarise(RFM_score=round(sum(RFM_score),5)) %>% dcast(ID ~ PD_S_NM)
A02_PD_H_NM_1 <- A02_PD_H_NM_1[,-1]
A02_PD_H_NM_1[is.na(A02_PD_H_NM_1)] <- 0
A02_PD_H_NM_1 <- ifelse(A02_PD_H_NM_1>0,1,0)
test_A02 <- apply(A02_PD_H_NM_1,2,sum) %>% as.data.frame 
test_A02$total <- nrow(A02_PD_H_NM_1)
test_A02$ratio <- round(test_A02$./test_A02$total,5)
test_A02 <- test_A02 %>% subset(ratio > 0.5) %>% rownames
A02_all_buy_black[[p]] <- test_A02
# A02_PD_H_NM_2<-A02_PD_H_NM
# A02_PD_H_NM_2 <- ifelse(A02_PD_H_NM_2>0,1,0)
# apply(A02_PD_H_NM_2,2,sum,na.rm=T)[-1]

target_user <- A02_PD_H_NM %>% select(ID,target[p])
target_user <- target_user[which(!is.na(target_user[,2])),]
A02_target_user[[p]] <- target_user[target_user[,2] > quantile(target_user[,2],0.75),]$ID
A02_future_user[[p]] <- target_user[target_user[,2] <= quantile(target_user[,2],0.75),]$ID

target_user <- SPI  %>% subset(BIZ_UNIT == "A02") %>% subset(ID %in% A02_target_user[[p]]) %>% subset(PD_H_NM==target[p]) %>% subset(!PD_S_NM %in% test_A02)
...
# Output neighbour results to a file  
  A02_item[[p]] <- data.germany.neighbours[,-1] %>% as.data.frame
...
################################################################ 
########### list : A02_data.germany.user.scores  ###############
################################################################
...
A02_data.germany.user.scores[[p]] <- holder
...
########################################################################## 
############# list : A02_data.germany.user.scores.holder  ################
##########################################################################
...
A02_data.germany.user.scores.holder[[p]] <- data.germany.user.scores.holder
}
```
...
###################################################################### 
####################### list : A02_user_popular  #####################
######################################################################


A02_user_popular <-list()
A02_user_popular.holder <- list()

for (p in 1:length(target)){
User_RFM_A02_S <- User_RFM_A02 %>% 
  subset(PD_H_NM==target[p]) %>% 
  subset(ID %in% A02_target_user[[p]]) %>% 
  dcast(ID ~ PD_S_NM)
User_RFM_A02_S[is.na(User_RFM_A02_S)]<-0
A02_user_popular[[p]] <- User_RFM_A02_S
A02_user_popular[[p]][A02_user_popular[[p]]==0.00000000]<-NA
rownames(A02_user_popular[[p]]) <- paste0('u',A02_user_popular[[p]]$ID)
A02_user_popular[[p]] <- A02_user_popular[[p]][,-1]
}
...
#############################################################################
####################### list : A02_user_popular.holder  #####################
#############################################################################


for (p in 1:length(target)){
  # Lets make our recommendations pretty
    data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A02_user_popular[[p]]),ncol=10,dimnames=list(rownames(nrow(A02_user_popular[[p]]))))

  for(i in 1:nrow(data.germany.user.scores.holder)) 
  {
    test_length <- A02_user_popular[[p]][,order(A02_user_popular[[p]][i,],decreasing=T)][i,] %>% length
    if(test_length>10) {
      test <- A02_user_popular[[p]][,order(A02_user_popular[[p]][i,],decreasing=T)][i,][1:10]
      }else{test <- head(n=10,A02_user_popular[[p]][,order(A02_user_popular[[p]][i,],decreasing=T)][i,])}
  ...
  A02_user_popular.holder[[p]] <- data.germany.user.scores.holder
  rownames(A02_user_popular.holder[[p]]) <- rownames(A02_user_popular[[p]])
}
```

# 5. A02 업종에서의 취향 순위
```{r}
A02_interest <- A02_PD_H_NM
A02_interest[,-1] <- apply(A02_interest[,-1],2,function(x)ifelse(x <= quantile(x,0.75,na.rm=T),NA,x))
A02_interest_s <- A02_interest
A02_interest_s[,-1] <- ifelse(A02_interest_s[,-1] > 0 ,1 ,0)
A02_interest$type_num <- apply(A02_interest_s[,-1],1,sum,na.rm=T)
A02_interest <- A02_interest %>% subset(type_num>0)

A02_interest_test <- A02_interest[,2:(ncol(A02_interest)-1)]
rownames(A02_interest_test) <- paste0('u',A02_interest$ID)

data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A02_interest_test),ncol=10,dimnames=list(rownames(nrow(A02_interest_test))))
 for(i in 1:nrow(data.germany.user.scores.holder)) {
    test_length <- A02_interest_test[,order(A02_interest_test[i,],decreasing=T)][i,] %>% length
    if(test_length>10) {
      test <- A02_interest_test[,order(A02_interest_test[i,],decreasing=T)][i,][1:10]
    }else{
        test <- head(n=10,A02_interest_test[,order(A02_interest_test[i,],decreasing=T)][i,])
        }
    
    data.germany.user.scores.holder[i,] <- c(names(test)[1:length(test[!is.na(test)])],rep(NA,10-length(names(test)[1:length(test[!is.na(test)])])))
}
  A02_interest.holder <- data.germany.user.scores.holder
  rownames(A02_interest.holder) <- rownames(A02_interest_test)
...
```{r} 
A02_interest.holder_S <- A02_interest.holder
for(i in 1:nrow(A02_interest.holder_S)){
    A02_interest.holder_S[i,]<-head(n=10,rep(A02_interest.holder_S[i,][!is.na(A02_interest.holder_S[i,])],10))
  }
  
 
data.germany.user.scores.holder <- matrix(NA, nrow=nrow(A02_interest.holder_S),ncol=100,dimnames=list(rownames(nrow(A02_interest.holder_S))))
rownames(data.germany.user.scores.holder) <- rownames(A02_interest.holder_S)
A02_user_popular.holder_copy <- A02_user_popular.holder
A02_data.germany.user.scores.holder_copy <- A02_data.germany.user.scores.holder

for (i in 1:nrow(A02_interest.holder_S)){
  k=1
  for(j in 1:ncol(A02_interest.holder_S)){
    u <- names(A02_interest.holder_S[i,j])
    p <- which(A02_interest.holder_S[i,j]==target)
     if(sum(rownames(A02_user_popular.holder_copy[[p]]) %in% u)==1){
       if(!is.na(A02_user_popular.holder_copy[[p]][u,1])){
      data.germany.user.scores.holder[i,k] <- A02_user_popular.holder_copy[[p]][u,1]
      A02_user_popular.holder_copy[[p]][u,] <- c(A02_user_popular.holder_copy[[p]][u,][-1],NA)
      k=k+1
    }}
    if(sum(rownames(A02_data.germany.user.scores.holder_copy[[p]]) %in% u)==1){
      if(!is.na(A02_data.germany.user.scores.holder_copy[[p]][u,1])){
      data.germany.user.scores.holder[i,k] <- A02_data.germany.user.scores.holder_copy[[p]][u,1]
      A02_data.germany.user.scores.holder_copy[[p]][u,] <- c(A02_data.germany.user.scores.holder_copy[[p]][u,][-1],NA)
      k=k+1
    }}
  }
}
...
A02_total_recomm <- data.germany.user.scores.holder
```

# 7. EDA : A02 업종에서 취향(대분류)별 성별, 나이 요약 정보
```{r}
...
# 8. EDA : A02 업종에서 개인이 가진 취향의 개수 분포
```{r}
# A02_PD_H_NM_test <- A02_PD_H_NM
# A02_PD_H_NM_test[,-1] <- lapply(A02_PD_H_NM[,-1],function(x) ifelse(x>quantile(x,0.75,na.rm=T),1,0))
# A02_PD_H_NM_test[is.na(A02_PD_H_NM_test)]<-0
# A02_PD_H_NM_test$type_num <- apply(A02_PD_H_NM_test[,-1],1,sum)
# 
# A02_PD_H_NM_test %>% ggplot(aes(x=as.factor(type_num))) + geom_bar() +  xlab("A02")
# A02_PD_H_NM_test$type_num %>% table
```
